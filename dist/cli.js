...
import { updateBarrels, watchBarrels, findBarrelDirs, } from "./index.js";
...
function cmdInitNonInteractive(dir) {
...
async function cmdInteractive(baseDir) {
    const resolved = resolveDir(baseDir);
    if (!fs.existsSync(resolved)) {
        console.error(`Directory not found: ${resolved}`);
        process.exit(1);
...
async function cmdInitInteractive(dir) {
    const resolved = resolveDir(dir);
    if (!fs.existsSync(resolved)) {
        console.error(`Directory not found: ${resolved}`);
        process.exit(1);
